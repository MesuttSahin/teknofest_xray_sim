...
# Proje kök dizinini görebilmesi için (src.utils import hatası almamak adına)
...
def main():
    ...
    if not os.path.exists(INPUT_FILE):
        ...
        print(f"   Çözüm: 'Data_Entry_2017_Mini.csv' dosyasını 'data/raw/' klasörüne taşıyın.\n")
        return
    ...
    # ---------------------------------------------------------
    # Hasta Bazlı Ayrıştırma (Patient ID Split)
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # GÜVENLİK KONTROLÜ (Data Leakage Check)
    # ---------------------------------------------------------
    ...
    print("🛡️  Güvenlik Kontrolü Geçildi: Veri sızıntısı yok (No Data Leakage).")

    # ---------------------------------------------------------
    # KAYIT İŞLEMİ
    # ---------------------------------------------------------

    # Dosyaları config.PROCESSED_DATA_DIR (data/processed) içine kaydet config kontrolü sağlandı görevde data/proceed diyordu ama sanırım yanlış yazılmış
